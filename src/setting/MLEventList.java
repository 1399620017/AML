...
public class MLEventList extends AsxConfig {
    ...
    protected void loadConfig(FileConfiguration config) {
        ...
        for (String key : rcm.setting.getbList()) {
            RcEvent eb =
					new RcEvent(key).setName(config.getString("box." + key + ".name")).setType(config.getString("box." + key + ".type")).setContent(config.getString("box." + key + ".content")).setNumber(config.getInt("box." + key + ".number")).setCmds(config.getStringList("box." + key + ".cmds")).setDesc(config.getStringList("box." + key + ".desc")).setPoint(config.getInt("box." + key + ".point")).setSlot(config.getInt("box." + key + ".slot")).setLevel(config.getInt("box." + key + ".level")).setPermission(config.getString("box." + key + ".permission", ""));
            bMap.put(key, eb);
        }
    }
